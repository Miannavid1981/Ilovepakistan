...
<section class="pt-4 mb-4">
    ...
    <div class="input-group">
        <input type="password" class="form-control rounded-0{{ $errors->has('password') ? ' is-invalid' : '' }}" 
               value="{{ old('password') }}" placeholder="{{ translate('Password') }}" name="password" id="yourPassword" required>
        <div class="input-group-append">
            <button type="button" class="btn btn-outline-secondary" id="toggleYourPassword">
                <i class="fa fa-eye" id="yourEyeIcon"></i>
            </button>
        </div>
    </div>
    ...
    <div class="input-group">
        <input type="password" class="form-control rounded-0" 
               placeholder="{{ translate('Confirm Password') }}" name="password_confirmation" id="repeatPassword" required>
        <div class="input-group-append">
            <button type="button" class="btn btn-outline-secondary" id="toggleRepeatPassword">
                <i class="fa fa-eye" id="repeatEyeIcon"></i>
            </button>
        </div>
    </div>
</div>
...
<script>
    document.getElementById('toggleYourPassword').addEventListener('click', function () {
        const yourPasswordInput = document.getElementById('yourPassword');
        const yourEyeIcon = document.getElementById('yourEyeIcon');

        // Toggle the type attribute
        if (yourPasswordInput.type === 'password') {
            yourPasswordInput.type = 'text';
            yourEyeIcon.classList.remove('fa-eye');
            yourEyeIcon.classList.add('fa-eye-slash');
        } else {
            yourPasswordInput.type = 'password';
            yourEyeIcon.classList.remove('fa-eye-slash');
            yourEyeIcon.classList.add('fa-eye');
        }
    });
</script>
...
<script>
    document.getElementById('toggleRepeatPassword').addEventListener('click', function () {
        ...
        if (repeatPasswordInput.type === 'password') {
            ...
            repeatEyeIcon.classList.remove('fa-eye');
            repeatEyeIcon.classList.add('fa-eye-slash');
        } else {
            repeatPasswordInput.type = 'password';
            repeatEyeIcon.classList.remove('fa-eye-slash');
            repeatEyeIcon.classList.add('fa-eye');
        }
    });
</script>
@endsection
