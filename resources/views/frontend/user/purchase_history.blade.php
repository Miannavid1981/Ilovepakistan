...
@section('panel_content')
    ...
    <div class="d-flex flex-column flex-sm-row justify-content-end align-items-end">
        <!-- Re-order -->
        <a class="btn-soft-white rounded-3 btn-sm mb-2 mb-sm-0 mr-sm-1"
            href="{{ route('re_order', encrypt($order->orders->first()->id)) }}" title="{{ translate('Re-order') }}">
            <i class="fas fa-sync-alt" style="font-size: 15px;"></i> <!-- Icon for reorder or refresh -->
        </a>
        ...
        @if ($order->orders->first()->payment_status == 'unpaid' && $order->orders->first()->payment_type != 'cod')
            ...
            <a class="btn-soft-white rounded-3 btn-sm mb-2 mb-sm-0 mr-sm-1"
                ...
                <img src="https://allaaddin.com/resources/views/frontend/user/payment-r.png" alt="Payment Image" style="width: 17px; height: 17px;"/> <!-- Image for payment -->
            </a>
        @endif
        ...
        @if ($order->orders->first()->delivery_status == 'pending' && $order->orders->first()->payment_status == 'unpaid')
            <a href="javascript:void(0)"
                class="btn btn-soft-danger btn-icon btn-circle btn-sm hov-svg-white mb-2 mb-sm-0 mt-sm-0 mr-sm-1 confirm-delete"
        ...
        <a href="{{ route('purchase_history.details', ['order_id' => encrypt($order->orders->first()->id), 'id' => encrypt($order->id)]) }}"
            class="btn btn-soft-info btn-icon btn-circle btn-sm hov-svg-white mb-2 mb-sm-0 mt-sm-0 mr-sm-1"
        ...
        <!-- Invoice -->
        <a class="btn btn-soft-warning btn-icon btn-circle btn-sm hov-svg-white mt-sm-0"
