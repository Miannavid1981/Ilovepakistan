...
@section('panel_content')
    ...
    @endif
        <div class=" test alert alert-danger" id="test" style="display:none;">
                    ...
                    <div class="card-body">
                        ...
                        <div id="show-hide-div">
                            ...
                            <div class="form-group row">
                               ...
                               <div class="col-md-6">
    <!-- Single input field that starts with the read-only SKU code -->
    <input type="text" placeholder="{{ translate('SKU') }}" id="sku-input" name="sku" class="form-control" required value="{{ $user->sku_code ?? '' }}">
</div>
                            </div>
                        </div>
                        <div class="form-group row">
            ...
            <div class="col-lg-4">
                ...
                <div class="card">
                    <div class="card-header">
                        ...
                        <h6 class="float-right fs-13 mb-0" style="margin-left: 36px;">
                            ...
                            <!--<p>Only Enabled categories will be shown here ! </p><br>--><br>
                                <a href="{{ url('/seller/profile') }}" >
                                    <strong>Please enabled categories from your profile section.</strong>
                                </a>
                        </h6>
                    </div>
                    <div class="card-body">
    <div class="h-300px overflow-auto c-scrollbar-light">
        <ul class="hummingbird-treeview-converter list-unstyled" data-checkbox-name="category_ids[]" data-radio-name="category_id">
            ...
            @endforeach
                ...
                <!--                    step="1" placeholder="{{ translate('Shipping Days') }}" required>-->
                <!--                <div class="input-group-prepend">-->
    ...
    <div class="card-body">
        <div class="form-group mb-3">
            ...
            <div class="input-group">
                <input type="text" class="form-control" name="est_shipping_days" placeholder="{{ translate('Enter shipping time e.g. 3 - 5 days') }}" required>
                <div class="input-group-prepend">
                    <span class="input-group-text" id="inputGroupPrepend">{{ translate('Days') }}</span>
                </div>
            </div>
...
<script type="text/javascript">
   ...
   document.addEventListener("DOMContentLoaded", function() {
    // Function to check corresponding radio button when checkbox is checked
    function checkCorrespondingRadioButton(checkbox) {
        if (checkbox.checked) {
            // Find the radio button with the same value and check it
            var radioButton = document.querySelector('input[type="radio"][value="' + checkbox.value + '"]');
            if (radioButton) {
    ...
    // Function to uncheck all checkboxes when a radio button is selected
    function uncheckAllCheckboxes(radioButton) {
        // Check corresponding checkbox
        var checkbox = document.querySelector('input[type="checkbox"][value="' + radioButton.value + '"]');
        if (checkbox) {
            checkbox.checked = radioButton.checked;
        }
        // Uncheck all other radio buttons
        var allRadioButtons = document.querySelectorAll('input[type="radio"][name="' + radioButton.name + '"]');
        allRadioButtons.forEach(function(otherRadioButton) {
    ...
    var checkboxes = document.querySelectorAll('#treeview input[type="checkbox"]');
    checkboxes.forEach(function(checkbox) {
        checkbox.addEventListener('change', function() {
            checkCorrespondingRadioButton(checkbox);
        });
    });

    // Add event listeners to radio buttons
    var radioButtons = document.querySelectorAll('#treeview input[type="radio"]');
    radioButtons.forEach(function(radioButton) {
        radioButton.addEventListener('change', function() {
            uncheckAllCheckboxes(radioButton);
        });
    });
});

document.getElementById('choice_form').addEventListener('submit', function(e) {
    var checkboxes = document.querySelectorAll('#treeview input[type="checkbox"]');
    var radio_button = document.querySelectorAll('#treeview input[type="radio"]');
    var isChecked_box = Array.from(checkboxes).some(checkbox => checkbox.checked);
    var isChecked_radio = Array.from(radio_button).some(radio_button => radio_button.selected);
    if (!isChecked_box ) {
        e.preventDefault();
...
<script>
document.addEventListener('DOMContentLoaded', function() {
    var form = document.getElementById('choice_form');
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const userSkuCode = "{{ $user->sku_code ?? '' }}";

        // Set the input value and ensure it starts with the userSkuCode
        skuInput.value = userSkuCode;
        skuInput.setSelectionRange(userSkuCode.length, userSkuCode.length); // Set cursor position after userSkuCode

        // Prevent editing of the initial read-only part
        skuInput.addEventListener('keydown', function (event) {
            const currentValue = skuInput.value;

            // If the cursor is in the read-only part, prevent backspace or editing
            if (skuInput.selectionStart < userSkuCode.length && (event.key === 'Backspace' || event.key === 'Delete' || event.key.length === 1)) {
                event.preventDefault();
                skuInput.setSelectionRange(userSkuCode.length, userSkuCode.length);
            }
        });

        // Update the input value dynamically to ensure the userSkuCode remains intact
        skuInput.addEventListener('input', function () {
            if (!skuInput.value.startsWith(userSkuCode)) {
                skuInput.value = userSkuCode + skuInput.value.slice(userSkuCode.length);
                skuInput.setSelectionRange(userSkuCode.length, skuInput.value.length);
            }
        });
    });
</script>
